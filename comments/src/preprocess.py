"""
Social media comments challenge — preprocessing.

Design goals
------------
- Be conservative: remove noise without destroying telecom signal.
- Work for multilingual / code-switched text:
  Arabic script + French + Darija/Arabizi (Latin).
- Mask obvious PII (phones, emails, URLs) and long numeric identifiers.
- Canonicalize a small set of telecom tokens (ADSL/FTTH/4G/modem/idoom...).

Important: For transformers, we keep the cleaned text as plain text (subword tokenizers
work best when we don't over-stem / over-tokenize).
"""
...
# Script boundary helpers (Arabic <-> Latin). This is important for code-switching like "لfibre".
...
def _remove_accents_latin(text: str) -> str:
    # NFKD splits accented chars into base + combining marks; Arabic letters are not affected.
...
def _looks_like_phone(digits_only: str) -> bool:
    ...
    # Normalize country-code prefixes.
...
def mask_pii(text: str, cfg: CommentsConfig) -> str:
    """
    Mask URLs / emails / phones / long IDs.
    """
    t = text
...
def reduce_elongations(text: str) -> str:
    """
    Reduce excessive Latin letter repetitions (common in social media).
    """
...
def clean_platform(value: Any) -> str:
    """
    Normalize platform values (Facebook, Twitter, Instagram...) safely.
    """
    ...
    # Collapse spaces and remove accents (platforms are Latin-based).
    v = _collapse_spaces(v)
    v = _remove_accents_latin(v)
    v = v.lower()

    # Fix obvious concatenations / typos we observed (e.g., "FacebookFacebook").
    v2 = v.replace(" ", "")
    ...
    # Standardize to snake-ish token.
...
def clean_comment(text: Any, cfg: CommentsConfig) -> str:
    """
    Main text cleaning entrypoint (single example).
    """
...
def preprocess_comments_df(df_raw: pd.DataFrame, cfg: CommentsConfig, *, is_train: bool) -> pd.DataFrame:
    ...
    # Standardize ID + label dtypes (model-friendly)
...
def assert_no_unmasked_phones(
    ...
    *,
    ...
    """
    DoD sanity check: after cleaning, no phone-like numbers should remain.
    """
    ...
    # If we already replaced with <PHONE>, ignore.
    candidates = s[s.str.contains(_PHONE_CANDIDATE_RE, regex=True, na=False) & ~s.str.contains(cfg.phone_token, regex=False, na=False)]
    bad = []
    for idx, txt in candidates.head(max_examples).items():
        # Verify it truly looks like a phone (avoid false positives like speed values)
        for m in _PHONE_CANDIDATE_RE.finditer(str(txt)):
            digits = re.sub(r"\D", "", m.group(0))
            if _looks_like_phone(digits):
                bad.append((idx, str(txt)))
                break
    if bad:
        msg = "\n".join([f"- idx={i}: {t[:200]}" for i, t in bad])
        raise ValueError(f"Unmasked phone-like numbers found after cleaning:\n{msg}")
