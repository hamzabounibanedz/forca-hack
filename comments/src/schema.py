...
Problem solved
--------------
The dataset headers are French and may appear "corrupted" depending on the environment
(e.g., `Réseau Social` may render as `R�seau Social` in some terminals).

We therefore:
- detect required columns by *normalized* name (accent-stripped, lowercased, punctuation removed)
- rename them to canonical names from `CommentsConfig`
"""
...
def read_csv_robust(path: str | Path, **kwargs: Any) -> pd.DataFrame:
    ...
    for enc in ("utf-8", "utf-8-sig", "cp1252", "latin-1"):
        try:
            ...
            last_err = e
            continue
    # Last resort (keeps the pipeline moving; better than crashing in Colab)
    try:
...
def guess_raw_to_canon(df: pd.DataFrame, cfg: CommentsConfig, *, is_train: bool) -> dict[str, str]:
    ...
    if id_raw is None:
        # fallback: the first integer-like column named close to id
...
def build_label_mapping(y_raw: pd.Series) -> tuple[dict[int, int], dict[int, int]]:
    ...
    - ids are 0..(n_classes-1) used internally for transformers + consistent probability ordering
    """
...
def run_all_checks(*, df_train: pd.DataFrame, df_test: pd.DataFrame, cfg: CommentsConfig) -> dict[str, Any]:
    """
    Minimal but strict checks to catch schema issues early.
    Returns a dict of summary stats (useful for run_summary.json).
    """
    out: dict[str, Any] = {}

    # Rename (does not mutate inputs)
    tr = rename_raw_columns(df_train, cfg, is_train=True)
    te = rename_raw_columns(df_test, cfg, is_train=False)

    required_train = {cfg.id_col, cfg.platform_col, cfg.text_col, cfg.target_col}
    required_test = {cfg.id_col, cfg.platform_col, cfg.text_col}

    missing_train = sorted(required_train - set(tr.columns))
    missing_test = sorted(required_test - set(te.columns))
    if missing_train:
        raise KeyError(f"Train missing required columns: {missing_train}")
    if missing_test:
        raise KeyError(f"Test missing required columns: {missing_test}")

    # ID uniqueness (critical for submission alignment)
    out["train_rows"] = int(len(tr))
    out["test_rows"] = int(len(te))
    out["train_id_unique"] = int(tr[cfg.id_col].nunique(dropna=False))
    out["test_id_unique"] = int(te[cfg.id_col].nunique(dropna=False))
    out["train_id_has_duplicates"] = bool(out["train_id_unique"] != out["train_rows"])
    out["test_id_has_duplicates"] = bool(out["test_id_unique"] != out["test_rows"])

    # Label sanity
    y = pd.to_numeric(tr[cfg.target_col], errors="coerce")
    out["label_missing"] = int(y.isna().sum())
    y_int = y.dropna().astype(int)
    out["n_classes"] = int(y_int.nunique()) if len(y_int) else 0
    out["labels_sorted"] = sorted(y_int.unique().tolist())
